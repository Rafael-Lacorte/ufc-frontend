...
function FighterHistory() {
  ...
  function simplifyResult(result: string): string {
    if (result.includes('KO/TKO')) return 'KOTKO';
    if (result.includes('Submission')) return 'SUB';
    if (result.includes('Decision')) return result.split(' - ')[1];
    return result;
  }
        ...
        <tbody>
          {fights.map(fight => (
            <tr key={fight.id} className={fight.isTitleFight ? 'title-fight' : ''}>
              ...
              <td>{simplifyResult(fight.result)}</td>
              <td>{fight.round}</td>
